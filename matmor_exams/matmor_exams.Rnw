...
\section{I wish more problems\ldots}

\begin{enumerate}
\item Give an example of 
\begin{enumerate}
\item a function $f(x,y)$ that has a non-zero gradient in all the points except the point $(1,3)$ and zero gradient in the point $(1,3)$
\item a function $g(x,y)$ that has no gradient on the line $x=2y$ and a non-zero gradient when $x\neq 2y$
\end{enumerate}
